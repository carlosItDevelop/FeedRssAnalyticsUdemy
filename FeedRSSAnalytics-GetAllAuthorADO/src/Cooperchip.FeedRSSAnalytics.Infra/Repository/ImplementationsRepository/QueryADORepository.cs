...
namespace Cooperchip.FeedRSSAnalytics.Infra.Repository.ImplementationsRepository
{
    public class QueryADORepository : IQueryADORepository
    {
        ...
        {
            var queryExcuted = ExecuteQuery("SELECT AuthorId, Author, COUNT(*) as Count FROM ArticleMatrices GROUP By AuthorId, Author ORDER By Author",
                null,
                reader => new Authors
                {
            ...
            return await Task.FromResult(queryExcuted);
        }
